        ...
        <figure>
            ...
            <div class="action-buttons">
                <div class="button-container">
                    ...
                    <a href="https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${record[Latitude]},${record[Longitude]}" class="action-btn" title="Street View" target="_blank">📷</a>
...
function selectOnMap(rec) {
    if (!rec || selectedRowId === rec.id) { return; }
    selectedRowId = rec.id;
    if (mode === 'single') {
        updateMap([rec]);
    } else if (lastRecords && Array.isArray(lastRecords)) {
...
grist.onRecord((record, mappings) => {
    ...
    if (mode === 'single') {
        ...
        if (!marker || !markersLayer) { return; }
        try {
            markersLayer.zoomToShowLayer(marker);
            marker.openPopup();
        } catch (e) {
            console.warn("Could not zoom to marker:", e);
        }
    }
});
...
grist.on('selection', (records) => {
    if (records.length === 1 && amap && popups) {
...
// Suppress Tippy.js warnings about undefined popper
if (typeof window !== 'undefined') {
    const originalWarn = console.warn;
    console.warn = function(...args) {
        if (args[0] && typeof args[0] === 'string' && args[0].includes('applyStyles')) {
            return; // Suppress Tippy.js warning
        }
